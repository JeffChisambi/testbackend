...
const traceCommodity = async (req, res, next) => {
  try {
    ...
    // 1. Search by Purchase Reference or ID
    ...
    if (purchases.length === 0) {
      ...
      if (grns.length > 0) {
        ...
        if (grn.purchase_id) {
          const [linkedP] = await query('SELECT * FROM purchases WHERE id = ?', [grn.purchase_id]);
          purchaseData = linkedP.length > 0 ? linkedP[0] : null;
        }

        return sendSuccess(res, 'Traceability chain retrieved via GRN Number', {
          search_term: term,
          grn_record: grn,
    ...
    // Build complete chain for matched purchase(s)
    const traceResults = [];
    ...
    for (const purchase of purchases) {
      ...
      const [deliveries] = await query(
        ...
        [purchase.commodity_id, purchase.variety_id]
      );
    ...
    return sendSuccess(res, 'Supply chain traceability records retrieved successfully', traceResults);
  } catch (error) {
    next(error);
  }
};
